def buscar_fecha(mayor_amp,menor_amp):
    lista=[]
    lista2=[]
    lista3=[]
...
with open('reporte.txt','w',encoding='UTF-8') as reporte:
    ...
    reporte.write('\n------------MAYOR Y MENOS AMPLITUD TERMICA EN UN DIA------------\n')
    ...
    reporte.write(f"Mayor amplitud: {mayor_amp[0]} con {mayor_amp[1]}°C el día {fecha[:2]} del {fecha[2:4]} del año {fecha[4:]}\n")
    reporte.write(f"Menor amplitud: {menor_amp[0]} con {menor_amp[1]}°C el día {fecha2[:2]} del {fecha2[2:4]} del año {fecha2[4:]}\n")
